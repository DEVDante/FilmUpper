...
W pliku wideo którego schemat przedstawiono na rysunku \ref{fig:fileStruckt} wyróżnić można następujące elementy: 
\begin{itemize}
...
\label{fig:MainInt}
\end{figure}
Interfejs prosi użytkownika o podanie pliku źródłowego oraz pliku docelowego (rysunek \ref{fig:MainInt}). W tym celu może on skorzystać z odpowiednich pól wpisując do nich ścieżki plików, wykonując akcję ,,przeciąg-upuść'' na te pola lub  korzystając z typowego okna wyboru pliku.
\begin{figure}
	%% add pic
\end{figure}
...
Po wykonaniu procesu przetwarzania użytkownik ma opcję porównania jakości filmu źródłowego z tym otrzymanym w wyniku działania wybranego algorytmu. W naszej aplikacji odbywa się to poprzez wyświetlenie obu filmów obok siebie w nowym oknie programu.
\begin{figure}
	%% add pic
\end{figure}
...
\end{verbatim}
a następnie wykonywana jest funkcjonalność slotów ,,podpiętych'' wcześniej do danego sygnału, co pokazano poniżej:
\begin{verbatim}
...
Głównym celem projektu jest takie zaprojektować naszą aplikację, aby możliwa była dalsza jej rozbudowa polegająca na dodawaniu nowych algorytmów. Struktura w jakiej organizowalibyśmy kod programu jest bardzo istotnym zagadnieniem z punktu widzenia przejrzystości systemu oraz ergonomii pracy z nim. Struktura naszego programu dzieli się na 2 główne części: część odpowiadającą za komunikację z użytkownikiem i algorytmami wybranymi przez użytkownika, oraz część polegającą na przetwarzaniu pliku wideo przez wybrane przez użytkownika algorytmy. Tworzyliśmy architekturę naszej aplikacji zgodnie z paradygmatem odwrócenia sterowania (IoC), polegającym na przekazywaniu gotowych obiektów do klas ich potrzebujących (rysunek \ref{fig:UMLStrukt}). Znacząco ułatwiło to projektowanie całej aplikacji a zwłaszcza jej testowanie.
...
Przed przystąpieniem do odczytu jakiegokolwiek pliku wideo przy pomocy bibliotek \emph{FFmpeg}-a musieliśmy wywołać funkcję 
\begin{verbatim}
av_register_all();
\end{verbatim}
...
public:
    ...
    virtual FrameEnhancerBase* Enhancer(
        IFrameReader* inputFrameReader,
        FilmQualityInfo* targetQualityInfo) { 
            return new FrameEnhancerBase(inputFrameReader,
        	                                targetQualityInfo); };
};
\end{verbatim}
...
public:
    ...
    virtual bool AreFramesLeft() 
        { return _frameEnhancer->AreFramesLeft(); };
};
\end{verbatim}
...
public:
    ...
    std::string Name;
    std::string Description;
    virtual FpsEnhancerBase* GetFpsEnhancer(
...
\section{Algorytmy}
Wykorzystywane przez nas algorytmy dzielą się na dwie poprzednio wyszczególnione kategorie -- przetwarzające pojedynczą klatkę obrazu (FrameEnhancerBase) oraz tworzące nowe klatki pośrednie (FpsEnhancerBase). Aby przedstawić efekty działania naszych algorytmów wykorzystaliśmy fragment filmu animowanego ,,Big Buck Bunny'' dostępnego na licencji Creative Commons (do prezentacji działania użyliśmy fragmentu od 0:47 do 0:53).
...
Na rysunku \ref{fig:handNN} widać duże, wyraźne piksele po przetworzeniu obrazu źródłowego. 


\subsubsection{Algorytm interpolacji dwuliniowej}
Algorytm interpolacji dwuliniowej polega na obliczeniu wynikowej wartości koloru na podstawie 4 pikseli otaczających (rysunek \ref{fig:interpolation1}).
...
Dzięki temu, otrzymane przez nas nowe piksele pośrednie znajdują się w jednej linii z pikselem wynikowym. Możemy teraz obliczyć wartość koloru piksela wynikowego używając wartości kolorów pośrednich do utworzenia nowej funkcji liniowej i wybrania jej wartości w miejscu przecięcia się funkcji z pikselem wynikowym (rysunek \ref{fig:interpolation3}). 
...
Na rysunku \ref{fig:handInterpolation} widać wygładzone krawędzie i bardzo gładkie przejścia między pikselami po przetworzeniu obrazu źródłowego.

\subsubsection{Algorytm interpolacji bi-kubicznej}
...
Na rysunku \ref{fig:handBiCubic} widać efekt podobny do efektu uzyskanego algorytmem interpolacji dwuliniowej (\ref{fig:handInterpolation}). Obraz jest podobnie wygładzony ale krawędzie są bardziej zaakcentowane.
...
\begin{enumerate}
	\item Obcinanie - jeżeli algorytm wymagałby do obliczenia piksela wartości znajdujących się poza krawędziami obrazu, piksel ten jest pomijany (rysunek \ref{fig:obcinanie}),
	\begin{figure}[h]
		\centering
		...
		\label{fig:obcinanie}
	\end{figure}
	\item Zawijanie - jeżeli wymagany jest piksel znajdujący się poza krawędziami obrazu, wartości te brane są z krawędzi przeciwnej (rysunek \ref{fig:zawijanie}),
	\newpage
	\begin{figure}[h]
		\centering
		...
		\label{fig:zawijanie}
	\end{figure}
	\item Rozszerzanie - krawędzie obrazu wejściowego rozszerzane są poprzez kopiowanie wartości pikseli brzegowych tyle razy ile wymaga tego algorytm (rysunek \ref{fig:rozszerzanie}).
	\begin{figure}[h]
		\centering
		\includegraphics[width=7cm]{extending.png}
		\caption{Obsługa pikseli brzegowych - rozszerzanie}
		\label{fig:rozszerzanie}
	\end{figure}
\end{enumerate}
...
Na rysunku \ref{fig:fpsInterlace} widać efekt podobny do tego, jaki został uzyskany przy zastosowaniu algorytmu interpolacji (rysunek \ref{fig:fpsIntBunny}), jednakże zamiast stopniowej zmiany wartości pikseli na nowe, zaktualizowane do nowych wartości zostały tylko co drugie wiersze pikseli obrazu.
...
\begin{verbatim}
    ...
    clock_t end = clock();
    double elapsed_secs = 
        (double(end - begin)/iterations) / CLOCKS_PER_SEC;
...
Niestety dalej było to o wiele za dużo względem tego czego oczekiwaliśmy po złożoności algorytmu (oczekiwaliśmy co najmniej kilkunastu klatek na sekundę). Następnymi podjętymi przez nas krokami było uproszczenie algorytmu poprzez usunięcie niektórych niepotrzebnych obliczeń. Uzyskaliśmy dzięki temu około $40\%$ przyśpieszenie. 
...
Uzyskana przez nas prędkość przetwarzania była zgodna z naszymi oczekiwaniami. Postanowiliśmy zatem skorzystać z przetwarzania danych z użyciem wielu wątków. Poprzez podział danych do przetworzenia na części zgodne z ilością wątków uzyskaliśmy dodatkowe przyśpieszenie. Z uwagi na koszt utworzenia wątków najlepsze wyniki uzyskaliśmy dla 2 wątków (czas przetwarzania klatki wynosił około 0.025 sekundy). Klatki te zostały przetworzone następującym kodem:
...
\begin{verbatim}
    ...
    {
        tt[t] = std::thread(CalculateFramePararel, 
        inputFrame, 
        outputFrame,
...
ustala liczbę wątków i tworzy ich tablicę (thread pool),

\begin{verbatim}
    for(int t = 0; t < threads; ++t)
    {
        tt[t] = std::thread(CalculateFramePararel, inputFrame,
        outputFrame,
        (_targetQualityInfo->Height / threads) * t, 
        (_targetQualityInfo->Height / threads) * (t + 1),
        _sourceQualityInfo, _targetQualityInfo);
...
rozpoczyna na każdym z wątków zadanie przetwarzania klatki na odpowiednim zakresie - od \verb|(_targetQualityInfo->Height / threads) * t|
do \\\verb|(_targetQualityInfo->Height / threads) * (t + 1))| przy użyciu metody  \\\verb|CalculateFramePararel|,
...
Mimo stworzenia aplikacji z myślą o dalszym rozwoju musieliśmy uprościć niektóre aspekty ze względu na ograniczające nas ramy czasowe. Najważniejszym zagadnieniem, które rozwijalibyśmy w pierwszej kolejności byłoby stworzenie rozwiązania ładującego dostępne algorytmy na podstawie folderu z nimi, gdzie zamieszczone byłyby implementacje algorytmów w postaci plików bibliotek dynamicznych (DLL).
...
\bibitem{CppBook}Bjarne Stroustrup, \emph{Programming: Principles and Practice Using C++ (2nd Edition)}, Addison-Wesley Professional, ISBN-13 978-0321543721
\bibitem{RustBook} Jim Blandy, Jason Orendorff, \emph{Programming Rust: Fast, Safe Systems Development}, O'Reilly Media, ISBN-13 978-1491927281
\bibitem{PythonBook} Eric Matthes, \emph{Python Crash Course: A Hands-On, Project-Based Introduction to Programming}, No Starch Press, ISBN-13 978-1593276034
\bibitem{DesignPatterns}     Erich Gamma, Richard Helm, Ralph Johnson, John Vlissides, \emph{Wzorce projektowe. Elementy oprogramowania obiektowego wielokrotnego użytku},  	Wydawnictwo Helion

\bibitem{NNScaling} Nearest Neighbor Image Scaling, http://tech-algorithm.com/articles/nearest-neighbor-image-scaling/ [dostępne: 11.01.2018]
\bibitem{InterpolationWiki}Interpolacja w przetwarzaniu danych wielowymiarowych, https://en.wikipedia.org/wiki/Multivariate\_interpolation [dostępne: 11.01.2018]
\bibitem{BicubicSpline} Cristian Constantin Lalescu, \emph{Two hierarchies of spline interpolations.  Practical algorithms for multivariate higher order splines.}, https://arxiv.org/pdf/0905.3564.pdf [dostępne: 11.01.2018]
\bibitem{YUVweb} Charles Poynton - Video engineering, http://poynton.ca/Poynton-video-eng.html [dostępne: 11.01.2018]
